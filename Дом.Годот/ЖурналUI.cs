...
public partial class ЖурналUI : ScrollContainer
{
    ...
    /// <summary>
    /// Новая жизнь — чистая лента.
    ///
    /// Не дописывать к прошлой: для героя это утро первого дня, и лента
    /// прошлой метели ему не видна. Что он помнит — это телефон
    /// и отголоски, и они отдельно.
    /// </summary>
    public void Новая(string заголовок)
    {
        _текст.Clear();
        _день = -1;
        Шапка(заголовок);
