...
public partial class ЗвукUI : Node
{
    ...
    private void Ветер()
    {
        ...
        if (сколько <= 0)
            return;
        // На дворе ветер тот же — только между ним и ухом нет стен.
        // Громкость по-прежнему считает симуляция; здесь всего лишь
        // перестаёт глушить перекрытие.
        float громко = (float)(_ветер_сила * (_на_улице ? 2.6 : 1.0));
        if (громко < Порог)
        {
