...
//
// **Мини-игры** (ГДД 7; `Дом.Ядро.МиниИгры`) — три вида в той же полосе:
//   · «Взлом» — удержание: метка за мышью в плывущей зоне, качество — доля
//     времени в зоне;
//   · «Ремонт» — тайминг: метка ходит туда-сюда, щелчок — один; чем ближе
//     к середине зоны, тем лучше; не щёлкнул — плохо;
//   · «Работа» — ритм: пять тактов, метка доходит до зоны на каждом,
//     щелчок в такт; промах или лишняя секунда — такт пропал.
// Щелчок — левой кнопкой мыши: клавиша действия здесь значит «бросить».
//
// **Удержание** подробнее:
...
public partial class ДелоUI : Control
{
    ...
    private double? _оценка;                     // тайминг и ритм: итог щелчков
    private double _центр = 0.5;                 // где стоит зона тайминга и ритма
    private readonly List<double> _щелчки = new();
    private const int ТАКТОВ = 5;
    private const double ТАКТ = 0.7, ДО_ТАКТА = 0.6;
    ...
    /// читает итог.</summary>
    public double качество => _оценка ?? (_играно > 0 ? _в_зоне / _играно : 0.5);

    /// <summary>Мини-игры, которые ловят щелчок: тайминг и ритм.</summary>
    private bool Щелчковая => _дело?.мини is Дом.Ядро.МиниИгра.РЕМОНТ or Дом.Ядро.МиниИгра.РАБОТА;
    ...
    public void Начать(Долгое дело, Предмет? цель, int день = 0)
    {
        ...
        _центр = 0.3 + System.Random.Shared.NextDouble() * 0.4;
        _где_метка = 0.5f;
        Visible = true;
        string как = дело.мини switch
        {
            Дом.Ядро.МиниИгра.ВЗЛОМ => "держи метку мышью в светлой полосе · ",
            Дом.Ядро.МиниИгра.РЕМОНТ => "щёлкни мышью, когда метка в светлой полосе · ",
            Дом.Ядро.МиниИгра.РАБОТА => "щёлкай мышью в такт — когда метка доходит до полосы · ",
            _ => "",
        };
        _как_бросить.Text = как + $"{Управление.В_скобках(Клавиши.ДЕЙСТВИЕ)} или [Esc] — бросить";
        Под_дорожку();
        Показать();
    }
    ...
    public void Кадр(double delta, Предмет? под_прицелом, bool можно, bool дом_ждёт)
    {
        ...
        if (!_дело.смотреть || ReferenceEquals(под_прицелом, _цель))
            _без_взгляда = 0;
    ...
    private void Под_дорожку()
    {
        ...
        // строка подсказки шире полосы (с мини-игрой в ней два указания) —
        // вдвое, по середине, чтобы не уезжала вправо
        _как_бросить.Position = new Vector2(-ШИРИНА / 2, ВЫСОТА + 3 + (есть ? ДОРОЖКА + 3 : 0));
        _как_бросить.Size = new Vector2(ШИРИНА * 2, 18);
    }
    ...
    /// <summary>
    /// Кадр тайминга и ритма: где метка, и — в конце — оценка. Тайминг:
    /// метка ходит туда-сюда (период 1.6 с), щелчок оценивает расстояние
    /// до середины зоны и кончает дело; не щёлкнул — 0.2. Ритм: метка
    /// доходит до зоны на каждом такте; щелчок — ближайшему несыгранному
    /// такту, допуск шире с зоной; оценка — среднее по тактам.
    /// </summary>
    private void Щёлкать()
    {
        double т = _прошло;
        double ширина = System.Math.Clamp(_дело!.зона, 0.05, 0.9);
        if (_дело.мини == Дом.Ядро.МиниИгра.РЕМОНТ)
        {
            _где_зона = (float)_центр;
            _где_метка = (float)(0.5 - 0.5 * System.Math.Cos(System.Math.Tau * т / 1.6));
            if (_щелчки.Count > 0 && _оценка is null)
            {
                double где = 0.5 - 0.5 * System.Math.Cos(System.Math.Tau * _щелчки[0] / 1.6);
                _оценка = System.Math.Clamp(1.0 - System.Math.Abs(где - _центр) / (ширина / 2 + 0.2), 0.0, 1.0);
                _прошло = _дело.секунд;           // щелчок один — дело кончено
            }
            else if (_оценка is null && т >= _дело.секунд - 0.05)
                _оценка = 0.2;                    // не решился — вышло криво
            return;
        }
        // ритм: следующий такт и где метка перед ним
        _где_зона = (float)_центр;
        double следующий = ДО_ТАКТА;
        while (следующий < т && следующий < ДО_ТАКТА + (ТАКТОВ - 1) * ТАКТ + 1e-9)
            следующий += ТАКТ;
        double до = System.Math.Clamp((следующий - т) / ТАКТ, 0.0, 1.0);
        _где_метка = (float)(_центр * (1.0 - до));
        if (т >= _дело.секунд - 0.05 && _оценка is null)
        {
            double допуск = 0.12 + ширина * 0.4;
            double сумма = 0;
            var свободные = new List<double>();
            for (int к = 0; к < ТАКТОВ; к++)
                свободные.Add(ДО_ТАКТА + к * ТАКТ);
            foreach (double щ in _щелчки)
            {
                if (свободные.Count == 0)
                    break;
                double ближний = свободные.OrderBy(б => System.Math.Abs(б - щ)).First();
                double ошибка = System.Math.Abs(ближний - щ);
                if (ошибка > допуск)
                    continue;                    // мимо такта — не в счёт
                сумма += 1.0 - ошибка / допуск * 0.6;
                свободные.Remove(ближний);
            }
            _оценка = System.Math.Clamp(сумма / ТАКТОВ, 0.0, 1.0);
        }
    }

    /// <summary>Щелчок левой кнопкой — тайминг и ритм; клавиша действия
    /// здесь по-прежнему «бросить».</summary>
    public override void _Input(InputEvent e)
    {
        if (!Visible || _дело is null || _ждёт || !Щелчковая)
            return;
