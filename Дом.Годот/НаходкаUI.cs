...
public partial class НаходкаUI : PanelContainer
{
    ...
    /// </summary>
    /// <summary>Что нашлось — строкой: ресурсы числом, еда — вещами
    /// («тушёнка свиная ×2, шпроты»), если она у героя вещи.</summary>
    public static string Что_нашлось(Добыча д, Продукты? продукты)
        => string.Join(" · ", д.вещи.Ключи
               .OrderBy(р => р, System.StringComparer.Ordinal)
               .Select(р => р == Продукты.ЕДА && продукты is not null && д.продукты is { Count: > 0 } вещи
                   ? Дом.Экран.Инвентарь.Словами(продукты, вещи)
                   : $"{р} {Текст.G(Текст.Округлить(д.вещи[р], 1))}"));

    public void Показать(Добыча д, Ноша ноша, Карман? карман, РучкиПетли ручки,
                         System.Action<Тара?> взять, System.Action оставить,
                         string где = "в коробке", Продукты? продукты = null)
    {
        _взять = взять;
        _оставить = оставить;
        _заголовок.Text = где.ToUpperInvariant();
        double всего = д.вещи.Ключи.Sum(р => д.вещи[р]);
        _что.Text = Что_нашлось(д, продукты)
