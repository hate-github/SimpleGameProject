...
public partial class ВопросUI : PanelContainer
{
    ...
    /// <summary>Разложить заново с теми же вариантами — после настроек:
    /// подсказка зовёт клавишу разворота по имени, а имя могло смениться.</summary>
    ...
    private void Разложить()
    {
        ...
        // клавишу разворота назначает игрок — подсказка зовёт её по имени
        string клавиша = Управление.Имя(Клавиши.СПИСОК);
        if (сколько < _текущий.варианты.Count)
            _варианты.AddChild(Строка(
                $"…и ещё {_текущий.варианты.Count - сколько}   ·   {клавиша}"));
        else if (!_всё)
            _варианты.AddChild(Строка($"{клавиша} — весь список с весами"));
    ...
    /// <summary>
    /// Клавишами тоже: цифра — вариант, клавиша списка (по умолчанию пробел) —
    /// развернуть и свернуть, Esc — свернуть. Мышь для тридцати вариантов
    /// в день — это тридцать движений туда и обратно.
    ///
    ...
    public override void _UnhandledInput(InputEvent e)
    {
        ...
        if (e is not InputEventKey к || !к.Pressed || к.Echo)
            return;
        // Esc при развороте сворачивает его, а не открывает настройки:
        // сперва закрывается то, что открыто
        if (_всё && к.Keycode == Key.Escape)
        {
            Свернуть();
            AcceptEvent();
            return;
