...
public partial class Витрина : Node3D
{
    ...
    private void Разобрать(Образец о, MeshInstance3D м, int зеркало)
    {
        ...
        // Наизнанку — по правилу, общему с игрой (`МоделиАвтора.Изнанка`):
        // вывернутое игра рисует с обеих сторон, и витрина обязана называть
        // вывернутым ровно то же
        var (вывернута, замкнута, доля) = МоделиАвтора.Изнанка(тр, м.GetAabb().GetCenter(), знак);
        if (вывернута)
        {
            о.наизнанку++;
    ...
    private static int Знак_куба() => МоделиАвтора.Знак_куба();
