...
public partial class Герой : CharacterBody3D
{
	...
	public override void _Ready()
	{
		...
		// сквозь неё видит, иначе коробку на верстаке не выбрать. Створка
		// входной двери и люди — тоже на своих слоях: в сетку ходьбы соседей
		// они не запекаются, а герой в них упирается как прежде.
		CollisionMask = 1 | Хранилище.СЛОЙ_МЕБЕЛИ | Мир.СЛОЙ_ДВЕРЕЙ | Мир.СЛОЙ_ЛЮДЕЙ;
		...
		_луч = new RayCast3D
		{
			...
			// сосед у двери — та же дверь: луч видит людей
			CollisionMask = 1 | Мир.СЛОЙ_ЛЮДЕЙ,
		};
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		else if (_луч.IsColliding() && _луч.GetCollider() is Node узел)
		{
			...
			// «дверь6» — дверь, «дверь6_валя» — Валя у шестой двери
			string номер = имя.Length > 5 ? имя[5..] : "";
			int черта = номер.IndexOf('_');
			if (черта >= 0)
				номер = номер[..черта];
			if (имя.StartsWith("дверь", StringComparison.Ordinal)
				&& int.TryParse(номер, out int кв))
				Перед_дверью = кв;
			else if (имя == "выход")
				Перед_выходом = true;
		}
