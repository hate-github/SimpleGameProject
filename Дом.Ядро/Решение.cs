...
public sealed class Ситуация
{
    ...
    /// <summary>С кем этот разговор или из-за кого ситуация: сосед у двери,
    /// вожак налёта, пойманный вор. Решению не нужен — он для того, кого
    /// спрашивают: игроку надо знать, кто перед ним.</summary>
    public NPC? с_кем { get; init; }

    /// <summary>О чём разговор — тоже только для спрошенного.</summary>
    public Суть? суть { get; init; }
}

/// <summary>
/// О чём вопрос: тема словом и то, чего она касается, — ресурс, место
/// вылазки, сосед, кладовая; у мены ещё и то, что дают взамен. Не текст:
/// слова для игрока собирает экран (`Дом.Экран.Разговор`), а ядро
/// только говорит, о чём речь. На ответ NPC не влияет ничем.
/// </summary>
public sealed record Суть(string тема, object? что = null, object? взамен = null);
...
public static class Решение
{
    ...
    public static bool монета(House h, NPC кто, Вопрос вопрос, string да, string нет,
                              double шанс, NPC? с_кем = null, Суть? суть = null)
    {
        var с = new Ситуация
        {
            вопрос = вопрос,
    ...
    public static bool по_правилу(House h, NPC кто, Вопрос вопрос, string да, string нет,
                                  double мерка, NPC? с_кем = null, Суть? суть = null)
    {
        var с = new Ситуация
        {
            вопрос = вопрос,
