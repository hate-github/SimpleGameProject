...
public static class Снимок
{
    ...
    private static readonly Dictionary<string, HashSet<string>> НЕ_СОСТОЯНИЕ =
        new(StringComparer.Ordinal)
        {
            ...
            {
                // `умения`, `карман`, `ноша`, `застали`, `снаряжение`, `шум_взлома`
                // и `без_сознания` — то же, что `решающий`:
                // приставлено к жильцу снаружи и в мире не существует.
                // У соседей их нет вовсе, и прогон без игрока даёт ровно
                // тот же снимок, что и раньше
                "shelter", "решающий", "умения", "карман", "ноша", "застали",
                "снаряжение", "шум_взлома", "без_сознания", "улицы", "приют", "сыграно",
            },
