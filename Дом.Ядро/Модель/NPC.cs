...
public sealed partial class NPC
{
    ...
    /// <summary>Улицы, какими их видит герой: куда замело так, что без
    /// снегоступов не пройти. Только у игрока, мимо снимка и сейва;
    /// у соседей null, и дорогу им считает сам дом.</summary>
    public IУлицы? улицы { get; set; }

    /// <summary>Где он греется, если не дома (участок, задание автора,
    /// пп. 17–18). Только у игрока, мимо снимка и сейва; у соседей null,
    /// и комнату им считает дом.</summary>
    public IПриют? приют { get; set; }

    /// <summary>Как сыграна мини-игра перед делом, которое дом сейчас
    /// исполнит (`МиниИгры`, «Ремонт», «Работа»). Только у игрока и на одно
    /// дело: `Действия.execute` сбрасывает. Мимо снимка и сейва.</summary>
    public Сыграно? сыграно { get; set; }
