...
public sealed class Ожидает
{
    ...
    // квартира → что и кто положил. Обычный словарь, а не запись: в прототипе
    // здесь тоже словарь, и снимок печатает его в порядке вставки (правило 6),
    // а поля записи — по имени (правило 7). Разница видна сразу
    public Словарь<int, Словарь<string, object>> подброшено { get; set; } = new();

    /// <summary>Собрать запись о подброшенном в том же порядке ключей,
    /// что и прототип: кто, что, день.</summary>
    public static Словарь<string, object> подброс(string кто, string что, int день)
    {
        var з = Словари.Строкой<object>();
        з["кто"] = кто;
        з["что"] = что;
        з["день"] = день;
        return з;
...
public sealed class House
{
    ...
    // сырой кусок lines.json, а не разобранная запись: у реплики быта есть
    // и «делает», и «кто», и условие вида «работает связь», — порт их терял,
    // потому что запись была беднее данных. В снимок не входит
    public System.Text.Json.JsonElement реплики_быт { get; set; }
