...
public sealed class Летопись
{
    ...
    /// <summary>Промзона «Вектор-3» (`Вектор`) — или null: её нет в мире.</summary>
    public ДанныеВектора? вектор { get; private set; }
    ...
    public static Летопись Прочитать(string каталог)
    {
        ...
        if (корень.TryGetProperty("вектор", out var вк))
            л.вектор = ДанныеВектора.Прочитать(вк);
