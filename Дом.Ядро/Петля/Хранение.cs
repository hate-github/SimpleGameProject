...
{
    /// <summary>
    /// Версия формата: наследие переживает не только жизни, но и правки
    /// кода, и когда-нибудь его придётся читать старым.
    ///
    /// Версия 2 добавила `знает_слово` (ГДД 11). Старое наследие читается
    /// по-прежнему — недостающее поле получает своё умолчание, и это
    /// не поблажка формату, а правило: **поле, появившееся позже, обязано
    /// иметь осмысленное умолчание, иначе его нельзя добавлять.** Здесь
    /// оно очевидное: кто не видел надписи — тот её и не видел.
    ///
    /// Без переноса вышло бы иначе, и это уже случилось однажды: поле
    /// добавили, версию не тронули, и сохранённая петля перестала
    /// открываться с сообщением «в наследии нет поля».
    ///
    /// Версия 3 добавила `знания` — что он знает о мире вокруг дома.
    /// Умолчание — ничего: кто не слышал знатоков, тот и не знает.
    /// </summary>
    public const int ВЕРСИЯ = 3;
    ...
    public static JsonObject Сложить(Наследие н)
    {
        ...
        return new JsonObject
        {
            ...
            ["знания"] = new JsonArray(н.знания.Select(з => (JsonNode?)з).ToArray()),
            ["записи"] = записи,
            ["отголоски"] = отголоски,
        };
    }
    ...
    public static Наследие Разобрать(JsonObject о)
    {
        ...
        if (версия < 1 || версия > ВЕРСИЯ)
            throw new InvalidDataException(
                $"наследие версии {версия}, а читаем от 1 до {ВЕРСИЯ}");
        ...
        var н = new Наследие
        {
            ...
            // версия 1 о слове не знала: значит не видел
            знает_слово = о["знает_слово"]?.GetValue<bool>() ?? false,
        };

        // версии 1 и 2 о мире вокруг дома не знали: значит ничего не знает
        if (о["знания"] is JsonArray знания)
            foreach (var з in знания)
                н.знания.Add(з!.GetValue<string>());
