    ...
    if h.day in (7, 14, 21, 28):
        for a in h.alive():
            for c in h.others(a):
                ...
                # «как он ко мне» — оценка по своему знанию (NPC.расположение_ко_мне)
                # против его настоящего доверия: наблюдатель читает правду,
                # решение — нет. Срезами по дням, как ошибка взгляда: по одним
                # выжившим в конце жизни пар слишком мало, и r гуляет
                РАСПОЛОЖЕНИЕ.append((a.расположение_ко_мне(c.id), c.trust.get(a.id, 3.0)))
...
РАСПОЛОЖЕНИЕ = []                                # (как он ко мне, по-моему; его доверие ко мне)
ВЫРОСЛО = [0, 0, 0]
СОЮЗЫ = []
ПРИГОВОРОВ = []
...
if len(РАСПОЛОЖЕНИЕ) > 2:
    try:
        r = st.correlation([x for x, _ in РАСПОЛОЖЕНИЕ], [y for _, y in РАСПОЛОЖЕНИЕ])
    except st.StatisticsError:
        r = float("nan")
    print(f"   «как он ко мне», по-моему, против его настоящего доверия: r = {r:.2f} "
          f"по {len(РАСПОЛОЖЕНИЕ)} парам в дни 7, 14, 21, 28")
print()
