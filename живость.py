...
for seed in range(1, N + 1):
    ...
    # против его настоящего доверия: наблюдатель читает правду, решение — нет
    for a in h.people.values():
        for c in h.people.values():
            if a.id != c.id and a.здесь() and c.здесь():
...
if len(РАСПОЛОЖЕНИЕ) > 2:
    ...
    print(f"   «как он ко мне», по-моему, против его настоящего доверия: r = {r:.2f} "
          f"по {len(РАСПОЛОЖЕНИЕ)} парам живых в конце жизни")
print()
