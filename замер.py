...
def main():
    ...
    # --- 4.1: погода как режим ---
    по_режиму = collections.Counter(self_режим for self_режим in н.режим_дня.values())
    пусто = collections.Counter()
    for (з, д), реж in н.режим_дня.items():
        if (з, д) not in н.вылазки:
            пусто[реж] += 1
    print("ПОГОДА (дней всего по режимам и доля дней, когда никто не вышел):")
    for реж in ("метель", "буран", "затишье"):
        всего_д = по_режиму.get(реж, 0)
        if not всего_д:
            print(f"  {реж:<9} — не случалось")
            continue
        print(f"  {реж:<9} {всего_д:>5} дн ({всего_д / n:.1f} за прогон), "
              f"никто не вышел в {100.0 * пусто.get(реж, 0) / всего_д:.0f}%")
    # чем дом занят в буран: он должен разговаривать и ссориться, а не отдыхать
    print("  чем дом занят (действий на день режима, топ-10 по бурану):")
    в_буран = по_режиму.get("буран", 0)
    в_метель = по_режиму.get("метель", 0)
    ключи = {k for (реж, k) in н.действия}
    строки = []
    for k in ключи:
        б = н.действия.get(("буран", k), 0) / max(1, в_буран)
        м_ = н.действия.get(("метель", k), 0) / max(1, в_метель)
        строки.append((б, k, м_))
    строки.sort(reverse=True)
    print(f"    {'действие':<14}{'буран':>8}{'метель':>9}")
    for б, k, м_ in строки[:10]:
        print(f"    {k:<14}{б:>8.2f}{м_:>9.2f}")
    print()
    ...
    доля_смертей = 100.0 * sum(1 for r in итоги if r["stats"].get("смертей_детей", 0)) / n
    брошено = 100.0 * sum(1 for r in итоги if r["stats"].get("детей_брошено", 0)) / n
    print(f"  ребёнок умер в {доля_смертей:.0f}% прогонов (цель 15-30), "
          f"смертей за прогон {смертей_детей:.2f}; брошен в {брошено:.0f}%")
    print()
