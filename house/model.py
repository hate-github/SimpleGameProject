...
class NPC:
    ...
    # день последнего открытого столкновения с этим соседом: осада у двери,
    # отъём на лестнице, засада. Не мнение и не история — состояние пары
    # «между нами теперь счёты». Пока его не было, налётчик заходил
    # к вчерашней жертве поговорить и меняться с ней банками: 13% всех
    # контактов после осады были разговором, 16% — обменом
    счёты: Dict[str, int] = field(default_factory=dict)
    ...
    def счёты_свежи(self, other_id: str, день: int, срок: int) -> bool:
        """Между нами ещё не улеглось: он ломал мою дверь на этой неделе."""
        было = self.счёты.get(other_id)
        return было is not None and день - было < срок
