...
def validate_data(balance, npcs, events, lines=None):
    ...
    for d in npcs["жильцы"]:
        ...
        # сутки жильца: во сколько встаёт и чем занимает своё время. Опечатка
        # в занятии — привычка, которой не соответствует ни одна строка быта:
        # человек будет «предпочитать» то, чего в доме не бывает, и разницы
        # не увидит никто
        подъём = d.get("подъём", 8.0)
        if not (5.0 <= float(подъём) <= 12.0):
            bad.append(f"{d['id']}: подъём в {подъём} — а бывает с 5 до 12")
        занятия = {в.get("занятие") for в in (lines or {}).get("быт", [])
                   if isinstance(в, dict) and в.get("занятие")}
        for отрезок, занятие in (d.get("привычки") or {}).items():
            if отрезок not in ("утро", "день", "вечер", "ночь"):
                bad.append(f"{d['id']}.привычки: нет такого отрезка дня «{отрезок}»")
            if занятие not in занятия:
                bad.append(f"{d['id']}.привычки: занятия «{занятие}» нет "
                           f"ни в одной строке быта (lines.json)")
