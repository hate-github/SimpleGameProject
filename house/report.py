# -*- coding: utf-8 -*-
"""Вывод: журнал дня, панель скрытых шкал и финальный разбор.

Чат жильцов живёт в chat.py: он не показывает дом, а меняет его.
...
def final_report(h, days, seed, w=None):
    ...
    if ушли:
        ...
        for p in sorted(ушли, key=lambda x: x.died_day or 0):
            ...
            if h.journal.secrets:
                хвост = ("   ⌁ " + vb(p.sex, "дошёл") if p.дошёл
    ...
    if h.кладовые:
        ...
        for к in sorted(h.кладовые.values(), key=lambda x: x.id):
            ...
            # как журнал
            ключи = sorted(p.form("gen") for p in h.people.values()
                           if к.id in p.ключи_кладовых)
            осталось = ", ".join(f"{r} {v:g}" for r, v in sorted(к.stock.items()) if v)
            # погреб и гараж оба мужского рода: «ВСКРЫТА» читалось как опечатка
            w(f"  {к.имя:<12} {'ВСКРЫТ' if к.вскрыта else 'заперт':<8} "
              f"ходок {к.ходок:<3} осталось: {осталось or 'пусто'}"
              + (f"; ключ у {', '.join(ключи)}" if ключи else "; ключ потерян")
              + (f"; числится за {хозяин.form('ins')}" if хозяин else ""))
    # то, что дом унёс с собой: кому и чего не простили. Это не диагностика,
    # это итог наравне с тем, кто выжил, — и читается он именно так
    счета = []
    for p_ in sorted(h.people.values(), key=lambda x: x.apt):
        for кому in sorted(p_.не_прощу):
            другой = h.get(кому)
            if другой is None:
                continue
...
def первые_дни(h):
    """Диагностика: в какой день дом впервые дошёл до налёта, смерти, союза, кражи, ухода."""
    s = h.stats
