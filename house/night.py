...
from .model import вещи, FIREARMS, Ночь, Вид
...
def решение(h, p):
    ...
    # сколько расправ он знает — один проход по памяти на всю ночь, а не
    # на каждую дверь (model.знает_расправ)
    расправ = p.знает_расправ()
    ...
    for t in h.others(p):
        ...
        score -= расправ[Вид.ВИДЕЛ_УБИЙСТВО] * b["страх_последствий_убийство"] / A
        score -= расправ[Вид.ВИДЕЛ_ИЗГНАНИЕ] * b["страх_последствий_изгнание"] / A
...
def consider_raid(h, npc):
    ...
    for t in h.others(npc):
        ...
        crew_size = len(conflict.recruit(h, npc, t, прикидка=True)[0])   # прикидка, а не зов: он ещё не пошёл
