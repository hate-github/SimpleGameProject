...
def решение(h, p):
    ...
    for t in h.others(p):
        ...
        score -= t.power() * 0.6  # правда: фаза 1: видимая_сила
...
def consider_raid(h, npc):
    ...
    for t in h.others(npc):
        ...
        fear = sum(p.power() for p in h.household(t)) * (1.4 - npc.t01("храбрость")) * 1.5  # правда: фаза 1: видимая_сила по всему хозяйству
        ...
        соло = (crew_size < b["налёт_минимум_группы"]
                and npc.power() < t.power() * b["налёт_соло_превосходство"])  # правда: фаза 1: видимая_сила
        if соло:
