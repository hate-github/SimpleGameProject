...
def засада(h, кто, жертва):
    ...
    social.поссорились(h, кто, жертва, b["счёты_тяжесть_отъёма"])
...
def run_siege(h, leader, target):
    ...
    for p in crew:
        social.поссорились(h, p, target, b["счёты_тяжесть_осады"])
    for p in crew:
    ...
    if (pay_ok and (fear > 0.5 or target.t01("храбрость") < 0.45 or страшно > 0.35)
        ...
        for p in crew:
            social.поссорились(h, p, target, b["счёты_тяжесть_мирная"])
