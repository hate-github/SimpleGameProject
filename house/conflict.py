...
from .decision import Вопрос, монета, по_правилу
...
def steal(h, thief, target):
    ...
    if caught_seen:
        ...
        if armed and монета(h, target, Вопрос.ВЫЙТИ_СО_СТВОЛОМ, "выйти", "не выходить", scare):
        ...
        escaped = монета(h, thief, Вопрос.БЕЖАТЬ_ОТ_ХОЗЯИНА, "бежать", "остаться",
        ...
        elif по_правилу(h, target, Вопрос.ДРАТЬСЯ_С_ВОРОМ, "драться", "отпустить",
...
def _verdict_taboo(h, eater):
    приговор_дома(h, eater, "людоедство",
                  f"то, что нашли у {'неё' if eater.sex == 'ж' else 'него'} в квартире")
...
def suspect(h, victim, exclude=None, real=None):
    ...
    for other in h.others(victim):
        ...
        # кого называли вором ПРИ НЁМ, того и подозревают: слово в общем чате
        # работает как наговор (GDD 14, тема «подозрения»). По своей памяти,
        # а не по счёту дома: разговор, которого он не слышал, на его мысли
        # не влияет (план агента, фаза 9)
        if victim.слышал_обвинение(other.id, h.day, b["чат_подозрение_дней"]):
            w += b["чат_подозрение_вес"]
        # голодного подозревают охотнее — того, кто голоден ПО МОИМ СВЕДЕНИЯМ:
        # оценка его запаса на его семью, и чем меньше я о нём знаю, тем ближе
...
def on_death(h, dead, killer=None, quiet=False, оружие=None, свидетели=None):
    ...
    Зовётся только из `умер`, поля смерти уже выставлены там.
...
def смерть_ребёнка(h, кто, р):
    ...
    for o in h.others(кто):
        опыт_о = кто.asking.get(o.id)
        отказов = опыт_о.отказали if опыт_о is not None else 0.0
        if отказов > 0:
...
def вернуть_детей(h):
    ...
    Не благотворительность. Если принявший считает мать не жильцом — то есть
    несколько дней подряд видел её такой, что списал со счетов (`не_жилец`), —
    ребёнок остаётся у него. Механизм тот же, которым дети переходят после
    смерти (`_orphan`), только повод другой и мать ещё жива.
    """
    b = h.B
    for мать in list(h.alive()):
        for р in list(мать.дети):
            ...
            # Нужно и то, что принявший считает мать не жильцом, и то, что
            # ему есть чем его кормить, и то, что жизнь для него уже
            # достаточно не своя, чтобы взять чужое живое
            оставит = 0.0
            if мать.id in хозяин.не_жилец:
                оставит = (хозяин.t01("лояльность") * b["невозврат_лояльность"]
                           + (1.0 - хозяин.normalcy) * b["невозврат_распад"]
                           + (b["невозврат_ценность"]
                              if хозяин.ценит("ребёнок") else 0.0)
            ...
            if оставит > 0:
                мать.дети.remove(р)
                мать.dependents = max(0, мать.dependents - 1)
                if not мать.дети:
                    мать.dependent_name = ""
                    мать.dependent_acc = ""
                хозяин.dependents += 1
                ...
                for w in h.others(хозяин):
                    if w.id != мать.id:
                        social.adjust(w, хозяин.id,
                                      trust=1.0 if w.ценит("ребёнок")
                                      else -1.0)
            else:
...
# Дубль catalog.ТЕГИ["обобрать"]: лежали здесь, чтобы не тянуть actions по кругу;
# цикла больше нет, и таблица уйдёт вместе с остальными в задаче 3
...
def recruit(h, leader, target, прикидка=False):
    """Состав группы ровно по GDD 16.

    `прикидка=True` — вожак только считает, с кем он мог бы пойти
    (`night.consider_raid`), и никого ещё не зовёт: вопрос соседям не задаётся.
    ...
    for p in h.others(leader):
        ...
        нейтрал = (p.panic >= b["налёт_нейтралы_порог"]
                   and p.сведения_о(target.id).aware >= b["налёт_нейтралы_порог"]
        ...
        # ворота нормальности — те же, что у вожака (night.consider_raid), и до сих пор
        ...
        # идти или отказать — решает тот, кого зовут (decision.py). Правило
        # прежнее, но теперь это его ответ на предложение (план агента, фаза 5г).
        # В прикидке (`consider_raid`) вопрос не задаётся: вожак ещё никого
        # не звал, и спрашивать игрока о том, чего не случилось, нельзя
        идёт = ((pull > порог) if прикидка
                else по_правилу(h, p, Вопрос.ПОЙТИ_В_НАЛЁТ, "пойти", "отказать", pull - порог))
        if идёт:
            crew.append(p)
        else:
            отказали.append(p)
...
def зов(h, leader, target, отказали):
    ...
    for p in отказали:
        ...
        # (decision.py); монета та же (план агента, фаза 5д)
        if хочу > 0 and монета(h, p, Вопрос.ПРЕДУПРЕДИТЬ_ЖЕРТВУ, "предупредить", "смолчать",
                               clamp(хочу / 3.5, 0.05, 0.85)):
            предупредил = p
...
def кричать(h, target, crew, b):
    ...
    не было вовсе: `defenders_of` считал за него, кто выйдет, а сам он не
    делал ничего. Между тем это ровно та минута, ради которой дом и строился:
    стоят у двери, и весь вопрос в том, откроет ли кто-нибудь свою.
    ...
    if not target.здесь():
        return False
    соседи = [p for p in h.others(target) if p.id not in {c.id for c in crew}]
    if not соседи:
        return False
    хочу = (max((target.боится(p.id) for p in crew), default=0.0) * b["крик_за_страх"]
            + target.panic / 100.0 * b["крик_за_панику"]
            # трое у двери — это уже не разговор через дверь
            + (len(crew) - 1) * b["крик_за_число"])
    # кого он зовёт: не дом вообще, а тех, кто, по его памяти, выходил
    # к другим и к кому он сам выходил
    хочу += max((target.свой(p.id) for p in соседи), default=0.0) * b["крик_за_своих"]
    ...
    return по_правилу(h, target, Вопрос.КРИЧАТЬ, "кричать", "молчать", хочу)
...
def defenders_of(h, target, crew_ids, предупреждён=False, поднял=None, крик=False):
    ...
    услышал их на лестнице и поднял дом; он и сам стоит у двери. `крик` —
    жертва позвала на помощь, и её слышал весь подъезд.
    """
    d = [target]
    ...
    for p in h.others(target):
        ...
        # в этой игре (`близость_за_защиту` 2.5), но сама близость на выход
        # не влияла ни на грамм: считались доверие, лояльность и храбрость,
        # то есть репутация и характер. Оттого за незнакомого выходили
        # с той же готовностью, что и за того, с кем полжизни у одной печки
        ...
        # у той же двери, в ту же минуту
        if target.id in p.не_вышли:
        ...
        if крик:
            # на крик выходят не потому, что стали храбрее, а потому, что
            # теперь это выбор, а не неведение: слышали все, и все знают,
            # что слышали все
        ...
        # и последнее слово за самим соседом (decision.py). Правило то же —
        # NPC выходит, когда перевесило, — но теперь это его решение, а не
        # вывод функции о нём: игрок в этой роли услышит, что за стеной
        # ломают дверь, и ответит сам (план агента, фаза 5а)
        if по_правилу(h, p, Вопрос.ВЫЙТИ_К_ДВЕРИ, "выйти", "остаться", will - порог):
            d.append(p)
    return d
...
def run_siege(h, leader, target):
    ...
    # и первое решение самой жертвы: закричать. До сих пор у того, к чьей
    # двери пришли, не было ни одного хода — за него всё считал defenders_of
    крикнул = кричать(h, target, crew, b)
    if крикнул:
        h.bump("криков")
        h.journal.line(f"{target.short} {vb(target.sex, 'закричал')} — так, что слышал весь подъезд.", 2)
        for w in h.alive():
            if w.id != target.id:
                social.adjust(w, target.id, aware=b["крик_осведомлённость"])
                if w.id not in crew_ids:
                    social.add_panic(w, b["крик_паника"])
        # и вожак это слышит. Пока не собрался подъезд, можно уйти: осада
        # не состоялась, но и цены за неё никто не заплатил
        уйти = (b["крик_вожак_уходит"]
                * (1.4 - leader.t01("храбрость"))
                / max(0.5, len(crew)))
        if монета(h, leader, Вопрос.УЙТИ_НА_КРИК, "уйти", "остаться", clamp(уйти, 0.0, 0.9)):
    ...
    # и то, что дом узнаёт о себе. Кто не вышел на крик — знает, что не вышел,
    # и знает об этом тот, кто кричал. Это не общая ненависть: односторонний
    # шаг близости вниз у самой жертвы, и он не возвращается сам собой.
    # Дом после осады другой не потому, что кто-то плохой, а потому, что
    # теперь известно, кто выходит, а кто нет
    if крикнул:
        вышли = {p.id for p in defenders}
        for p in h.others(target):
            if p.id in вышли or p.id in crew_ids:
                continue
            social.отдалились(target, p.id, b["близость_за_невыход"])
            target.не_вышли.add(p.id)

    if len(defenders) > 1:
        h.bump("осад_с_защитой")
        h.bump("защитников", len(defenders) - 1)
    ...
    # ворота откупа стали склонностью (план поведения, шаг 7). Напуганный,
    # трусливый или знающий этот топор бросает монету, как и прежде. Смелый
    # и богатый раньше об откупе не думал вовсе — теперь вопрос задаётся
    # и ему, а отвечает он по ручке откуп_смелого: при нуле — правилом,
    # без броска (поведение прежнее); больше нуля — монетой с урезанной
    # склонностью. Значение ручки — за автором
    напуган = fear > 0.5 or target.t01("храбрость") < 0.45 or страшно > 0.35
    if pay_ok and напуган:
        откупается = монета(h, target, Вопрос.ОТКУПИТЬСЯ, "откупиться", "не платить", готов_платить)
    elif pay_ok and b["откуп_смелого"] > 0:
        откупается = монета(h, target, Вопрос.ОТКУПИТЬСЯ, "откупиться", "не платить",
                            clamp(готов_платить * b["откуп_смелого"], 0.0, 0.95))
    elif pay_ok:
        откупается = по_правилу(h, target, Вопрос.ОТКУПИТЬСЯ, "откупиться", "не платить", 0.0)
    else:
        откупается = False
    if откупается:
    ...
    if монета(h, target, Вопрос.ГОВОРИТЬ_ЧЕРЕЗ_ДВЕРЬ, "говорить", "молчать",
    ...
    if armed:
        ...
        if монета(h, leader, Вопрос.УЙТИ_ОТ_СТВОЛА, "уйти", "лезть дальше",
    ...
    if держит or прочность >= 2.0:
        ...
        if монета(h, target, Вопрос.БЕЖАТЬ_ЧЕРЕЗ_ОКНО, "бежать", "остаться", clamp(уйти, 0.0, 0.9)):
        ...
        if (not держит and target.weapon != Оружие.НЕТ and target.t01("храбрость") > 0.55
                and монета(h, target, Вопрос.ЖДАТЬ_ЗА_ДВЕРЬЮ, "ждать", "не ждать", b["засада_шанс"])):
    ...
    # Что на кону (план поведения, шаг 8): ребёнок за спиной и последняя еда
    # сдвигают пороги правила — трусости и превосходства. Знак каждой ручки
    # за автором: плюс к ребёнку — с ребёнком сдаются охотнее, минус к голоду —
    # с последней едой дерутся отчаяннее. При нулях поведение прежнее
    трусость = b["сдаться_трусость"] + (b["сдаться_за_ребёнка"] if target.dependents else 0.0)
    голоден = target.days_of("еда") < b["сдаться_голод_дней"]
    превосходство = b["сдаться_превосходство"] * (1.0 + (b["сдаться_за_голод"] if голоден else 0.0))
    по_силам = (def_power < attack_power * превосходство
                or target.t01("храбрость") < трусость)
    surrender = по_правилу(h, target, Вопрос.СДАТЬСЯ, "сдаться", "драться", 1.0 if по_силам else 0.0)
    ...
    if surrender:
        ...
        if (решимость > b["добить_решимость"] and not target.dependents
                and монета(h, leader, Вопрос.ДОБИТЬ, "добить", "оставить", b["добить_шанс"])):
            leader.bump("убийств")
            h.bump("убийств")
        ...
        if (хочу_его_квартиру(h, leader, target) > 0
                and монета(h, leader, Вопрос.ЗАНЯТЬ_КВАРТИРУ, "занять", "уйти", b["занять_после_налёта"])):
        ...
        # сначала — есть ли у жертвы ребёнок: мать на мороз не выставляют,
        # и спрашивать об этом вожака незачем (план поведения, шаг 6). Раньше
        # монета бросалась до этой проверки, и ответ пропадал: для NPC исход
        # тот же, для человека — вопрос впустую, для ленты случайности — сдвиг
        if ((leader.trait("вспыльчивость") >= 7 or leader.hate.get(target.id, 0) > 70)
                and not target.dependents
                and монета(h, leader, Вопрос.ВЫСТАВИТЬ_НА_МОРОЗ, "выставить", "оставить",
...
def занять_силой(h, кто, у_кого):
    ...
    h.journal.line(f"{кто.short} {vb(кто.sex, 'перебрался')} в кв.{новая.apt}. "
                   + (f"{у_кого.short} {vb(у_кого.sex, 'ушёл')} в его прежнюю, кв.{старая.apt}."
                      if у_кого.здесь() else "Хозяина больше нет."), 2)
