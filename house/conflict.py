...
def recruit(h, leader, target, прикидка=False):
    ...
    for p in h.others(leader):
        ...
        идёт = ((pull > порог) if прикидка
                else по_правилу(h, p, "пойти в налёт", "пойти", "отказать", pull - порог))
        if идёт:
            crew.append(p)
        else:
            отказали.append(p)
...
def зов(h, leader, target, отказали):
    ...
    for p in отказали:
        ...
        if хочу > 0 and монета(h, p, "предупредить жертву", "предупредить", "смолчать",
...
def defenders_of(h, target, crew_ids, предупреждён=False, поднял=None, крик=False):
    ...
    for p in h.others(target):
        ...
        if по_правилу(h, p, "выйти к двери", "выйти", "остаться", will - порог):
            d.append(p)
    return d
...
def run_siege(h, leader, target):
    ...
    if pay_ok and напуган:
        откупается = монета(h, target, "откупиться", "откупиться", "не платить", готов_платить)
    elif pay_ok and b["откуп_смелого"] > 0:
        откупается = монета(h, target, "откупиться", "откупиться", "не платить",
                            clamp(готов_платить * b["откуп_смелого"], 0.0, 0.95))
    elif pay_ok:
        откупается = по_правилу(h, target, "откупиться", "откупиться", "не платить", 0.0)
    else:
        откупается = False
    if откупается:
