...
def вернуть_детей(h):
    ...
    for мать in list(h.alive()):
        for р in list(мать.дети):
            хозяин = h.get(р.get("у")) if р.get("у") else None
            р["у"] = None
            ...
            if мать.id in хозяин.не_жилец:
                оставит = (хозяин.t01("лояльность") * b["невозврат_лояльность"]
                           ...
                           + (b["невозврат_ценность"]
                              if "ребёнок" in (хозяин.values or {}).get("ценит", ()) else 0.0)
                ...
                for w in h.others(хозяин):
                    if w.id != мать.id:
                        social.adjust(w, хозяин.id,
                                      trust=1.0 if "ребёнок" in (w.values or {}).get("ценит", ())
                                      else -1.0)
            else:
...
def кричать(h, target, crew, b):
    ...
    """
    if not (target.alive and not target.exiled):
        return False
