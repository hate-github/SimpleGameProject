...
def build_house(h, npcs_data):
    ...
    for d in npcs_data["жильцы"]:
        ...
        p._h = h
        # кто за него решает (GDD 13): по умолчанию мягкий выбор NPC; «скрипт»
        # и «человек» — для сценариев и игрока, правила у всех одни
        p.решающий = РЕШАЮЩИЕ[d.get("решает", "softmax")]()
        ...
        h.people[p.id] = p
    # стартовые отношения
    # чужой шкаф каждый оценивает по себе (ДОГАДКА), но у подъезда есть и то,
    # что он видел своими глазами задолго до метели: кто всю осень таскал
    # в подвал мешки, а кто въехал в пустую квартиру с рюкзаком. Это не догадка
    # и не слух, а знание, с которым дом входит в первый день, — поэтому оно
    # лежит в данных того, о ком известно, а не пятнадцатью строчками у соседей
    известен = {d["id"]: (d.get("известен_дому") or {}) for d in npcs_data["жильцы"]}
    for d in npcs_data["жильцы"]:
        p = h.people[d["id"]]
        ...
        for other in h.people.values():
            ...
            слава = известен.get(other.id, {})
            p.сведения[other.id] = Сведения(
                est={"еда": 3.0, "топливо": 3.0, "лекарства": 0.5, "материалы": 1.0}
                    | {k: float(v) for k, v in (слава.get("запас") or {}).items()},
                aware=float(слава.get("осведомлённость",
                                      15.0 + (10.0 if other.floor == p.floor else 0.0))))
