# -*- coding: utf-8 -*-
"""Утренние обнаружения: то, что ночью положили в h.ожидает, утром разбирают.
...
def вскрытые_кладовые(h):
    ...
    очередь, h.ожидает.вскрытые_кладовые = h.ожидает.вскрытые_кладовые, []
    for вор_id, kid in очередь:
...
def подброшенное(h):
    ...
    очередь, h.ожидает.подброшено = h.ожидает.подброшено, {}
    for apt, что in list(очередь.items()):
...
def пропажи(h):
    ...
    losses, h.ожидает.пропажи = h.ожидает.пропажи, []
...
def огласить_непрощённых(h):
    ...
    """
    накоплено, h.ожидает.не_простил = h.ожидает.не_простил, {}
    if not накоплено:
        return
