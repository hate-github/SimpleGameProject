...
# Что именно выдаёт тот или иной звук/запах (GDD 13).
# (ресурс, на сколько подрастает оценка, как это выглядит со стороны)
NOISE_MEANING = {
    "готовка": (None, 0.0, "в кв.{apt} гремят посудой"),
    "буржуйка": ("топливо", 1.1, "из окна кв.{apt} идёт дым"),
    "генератор": ("топливо", 2.2, "в кв.{apt} гудит генератор"),
    "ремонт": ("материалы", 1.2, "в кв.{apt} стучат"),
    "дверь": ("материалы", 1.4, "в кв.{apt} что-то тяжёлое волокут к двери"),
    "разбор": ("материалы", 1.0, "с пустой квартиры тащат доски"),
    "возвращение": ("еда", 1.6, "{кто} вернулся с улицы с пакетами"),
    "взлом": (None, 0.0, "в подъезде скрипела чужая дверь"),
    "ссора": (None, 0.0, "на лестнице кричали"),
    "выстрел": (None, 0.0, "в доме стреляли"),
    "шаги": (None, 0.0, "на лестнице шаги"),
}
...
def emit(h, src, level, kind, night=False, text_for=None):
    ...
    for other in h.others(src):
        if other.away:
            continue
        p = base
        p *= b["шум_затухание_на_этаж"] ** h.floor_gap(src, other)
        if night:
            # ночью тихо: слышно лучше, но спящий может не проснуться
...
def _hear(h, listener, src, kind, level):
    ...
    b = h.B
    res, hint, _ = NOISE_MEANING.get(kind, (None, 0.0, ""))
...
def gossip(h, a, b_npc):
    ...
    bal = h.B
    weight = 0.25 + 0.05 * a.trust.get(b_npc.id, 3.0)
    for third in h.others(a):
        ...
        if a.aware.get(third.id, 0) > b_npc.aware.get(third.id, 0) + 12:
            ...
            for res in ("еда", "топливо", "лекарства"):
                note_signal(b_npc, third.id, res, a.believed(third.id, res), weight * 0.6)
        # и наоборот
        if b_npc.aware.get(third.id, 0) > a.aware.get(third.id, 0) + 12:
            adjust(a, third.id, aware=bal["осведомлённость_за_слух"])
            for res in ("еда", "топливо", "лекарства"):
                note_signal(a, third.id, res, b_npc.believed(third.id, res), weight * 0.6)
...
def daily_decay(h):
    """Память притупляется, злость немного оседает."""
    b = h.B
    for p in h.alive():
