...
from .model import Оружие, Вид
from .hooks import Хуки

_ВИДЫ_ПАМЯТИ = set(Вид)
...
def invariants(h):
    ...
    for p in h.people.values():
        ...
        # память — единственный след того, что человек видел и слышал, и она
        # читается по виду (NPC.знает_о_кражах, NPC.слышал, conflict.suspect).
        # Опечатка в виде тихо ничего не найдёт, поэтому вид — из перечисления,
        # а «о ком» — существующий жилец
        for m in p.memory:
            if m.вид not in _ВИДЫ_ПАМЯТИ:
                say(f"{p.short}: запись памяти неизвестного вида «{m.вид}»")
            if m.кто not in h.people:
                say(f"{p.short}: память о несуществующем «{m.кто}»")
            if m.день > h.day:
                say(f"{p.short}: запись памяти из будущего (день {m.день})")
