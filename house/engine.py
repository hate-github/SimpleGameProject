...
class Simulation:
    ...
    def _day(self, h):
        ...
        часами (`NPC.час`) день стал лентой: ранняя пташка успевает первой,
        поздний застаёт полки пустыми, и «кто в это время на лестнице»
        впервые имеет ответ. Жребий остался — но только для тех, у кого
        на часах одно и то же: очередь внутри одного часа не заслуга.
        """
        guard = 0
        while guard < 600:
            guard += 1
            готовые = [p for p in h.alive()
                       if p.time_left >= 0.3 and p.health > 0 and p.здесь()]
            if not готовые:
                break
            жребий = {p.id: i for i, p in enumerate(h.rng.shuffled(готовые))}
            p = min(готовые, key=lambda x: (x.час(), жребий[x.id]))
            if not actions.choose_and_do(h, p):
